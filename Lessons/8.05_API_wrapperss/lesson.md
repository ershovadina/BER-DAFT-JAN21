...
- A Wrapper is any framework or entity that is used to mask or wrap another object. It is mainly used for two reasons: First, it helps you convert data into a compatible format. .
-- It helps you convert data into a compatible format (*Formatting*)
-- It helps you simplify a complex entity or makes a program perform complicated tasks easily (*Generalizing*)
...
Create a list and assign it to a variable named `my_20_artists`, loop over your 20 favourite artists and their results to the list. Create a function that takes a list of artist names and return their 50 first appearances as a dictionary.
...
Create a function to exctract all the artists of a song and another function to extract all the artists IDs from a song.
...
Now that we have all the tracks from a plylist lets get all the artists:
```python
...
```python
songs = {track["track"]["id"]: [track["track"]["name"], track["track"]["artists"][0]["name"]] for track in playlist["items"]}
```
...
```python
coldplay_songs = get_track_ids_from_album(coldplay_album_ids)
...
Create a function to get all the songs from all the artists of the playlist we worked with before
